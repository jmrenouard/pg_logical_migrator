...
class Migrator:
    ...
    def step4a_migrate_schema_pre_data(self, drop_dest=False):
        ...
        dump_cmd = (f"PGPASSWORD='{src_pass}' pg_dump -h {src_host} -p {src_port} -U {src_user} "
                    f"-s --section=pre-data {schema_args} {src_db}")
        psql_cmd = (f"PGPASSWORD='{dst_pass}' psql -v ON_ERROR_STOP=0 --echo-all "
    ...
    def get_initial_copy_progress(self):
        ...
        self.replication_cfg['subscription_name']
        pub_name = self.replication_cfg['publication_name']
