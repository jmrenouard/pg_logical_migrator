...
class MigratorApp(App):
    ...
    #cmd_progress { background: #d35400; color: white; }
    ...
    #step_4a { background: #e67e22; color: white; }
    #step_4b { background: #e67e22; color: white; }
    ...
    #step_15 { background: #8e44ad; color: white; }
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="main_container"):
            with VerticalScroll(id="sidebar"):
                ...
                yield Button("4a. Copy Schema Pre-data", id="step_4a")
                ...
                yield Button("➤ Initial Copy Progress", id="cmd_progress")
                yield Button("7. Replication Status", id="step_7")

                yield Label("── FINALIZATION ──", classes="section_label")
                yield Button("8. Refresh MatViews", id="step_11")
                yield Button("9/10. Sync Sequences", id="step_8")
                yield Button("11. Enable Triggers", id="step_10")
                yield Button("➤ Disable Triggers", id="cmd_disable_triggers")
                yield Button("12. Copy Schema Post-data", id="step_4b")
                yield Button("13. Reassign Ownership", id="step_15")

                yield Label("── VALIDATION & CLEANUP ──", classes="section_label")
                yield Button("14. Object Audit", id="step_13")
                yield Button("15. Row Parity", id="step_14")
                yield Button("16. STOP / CLEANUP", id="step_12")
        ...
        try:
            if event.button.id == "step_1":
                ...
                if sizes and sizes.get("database"):
                    size_header = f"\n\nSIZE ANALYSIS - Total DB Size: {sizes['database'].get('total_pretty', 'Unknown')}\n"
                ...
                self.result_area.update(Panel(msg, title="[Step 4a] Schema PRE-DATA Migration", border_style=color))
                self._log_detail("Schema Pre-Data", cmds, outs)
                self.log_area.write(f"  Result: {'OK' if success else 'FAIL'} — {msg}")

            elif event.button.id == "step_4b":
                success, msg, cmds, outs = self.migrator.step4b_migrate_schema_post_data()
                color = "green" if success else "red"
                self.result_area.update(Panel(msg, title="[Step 4b] Schema POST-DATA Migration", border_style=color))
                ...
                if not progress or not progress.get("summary"):
                    self.result_area.update(Panel("Could not fetch progress. Is subscription created and data sync started?", title="Progress", border_style="red"))
                else:
                    summ = progress.get("summary", {})
                    info = f"Overall Progress (Bytes): {summ.get('percent_bytes', 0)}%\n"
                    info += f"  {summ.get('bytes_copied_pretty', '0 B')} / {summ.get('total_source_pretty', '0 B')} copied\n\n"
                    info += f"Table Progress: {summ.get('percent_tables', 0)}%\n"
                    info += f"  {summ.get('completed_tables', 0)} / {summ.get('total_tables', 0)} ready"
                    ...
                    for r in progress.get("tables", []):
                        ...
                        table.add_row(
                            str(r.get('table_name', 'unknown')), 
                            f"[{color}]{state}[/{color}]",
                            f"{pretty_size(r.get('bytes_copied', 0))} / {pretty_size(r.get('size_source', 0))}",
                            f"{r.get('percent', 0)}%"
                        )
                    
                    # Wrap table in a Panel, use summary info as title
                    title = f"Bytes: {summ.get('percent_bytes', 0)}% ({summ.get('bytes_copied_pretty', '0 B')}/{summ.get('total_source_pretty', '0 B')}) | Tables: {summ.get('completed_tables', 0)}/{summ.get('total_tables', 0)}"
                    self.result_area.update(Panel(table, title=title, border_style="blue"))
                    self.log_area.write(f"  Progress: {summ.get('percent_bytes', 0)}% bytes, {summ.get('percent_tables', 0)}% tables.")
                ...
                self.result_area.update(Panel(msg, title="[Step 15] Reassign Ownership", border_style=color))
                self._log_detail("Reassign Ownership", cmds, outs)
                self.log_area.write(f"  Result: {'OK' if success else 'WARN'} — {msg}")
    ...
    def _do_init_async(self, label: str):
        ...
        try:
            ...
            try:
                ...
                pass
            s, m, c, o = self.migrator.step4a_migrate_schema_pre_data(drop_dest=drop_dest)
            if not s:
                 log_msg(f"✘ Schema PRE-DATA migration failed: {m}")
                 update_ui("Init Replication Failed", m, "red")
                 return
            log_msg("✔ Step 4a: Schema PRE-DATA migration completed.")
            ...
            s2, m2, c2, o2, r2 = self.validator.compare_row_counts(use_stats=use_stats)
            log_msg(f"  - Row Parity ({'stats' if use_stats else 'count'}): {'OK' if s2 else 'FAIL'}")
    ...
    def _do_post_async(self, label: str):
        ...
        try:
            ...
            res = self.checker.check_connectivity()
            if not (res['source'] and res['dest']):
                 log_msg("✘ Connectivity check failed. Aborting.")
                 update_ui("Post Migration Failed", "Connectivity check failed.", "red")
                 return
            log_msg("✔ Step 1: Connectivity check passed.")

            # NEW: Wait for final sync before stopping
            log_msg("▶ Ensuring all data is synchronized before stopping replication...")
            s_sync, m_sync, _, _ = self.migrator.wait_for_sync(timeout=3600)
            if not s_sync:
                log_msg(f"✘ Data synchronization timed out: {m_sync}")
                update_ui("Post Migration Failed", m_sync, "red")
                return
            log_msg("✔ Final data synchronization completed.")
            
            # Step 12: Terminate Replication
            s, m, c, o = self.migrator.step12_terminate_replication()
            self.call_from_thread(self._log_detail, "Cleanup Replication", c, o)
            if not s:
                log_msg(f"✘ Replication cleanup failed: {m}")
                update_ui("Post Migration Failed", m, "red")
                return
            log_msg("✔ Step 12: Replication cleaned up.")
            
            # Step 4b: Schema Post-Data
            s0, m0, c0, o0 = self.migrator.step4b_migrate_schema_post_data()
            self.call_from_thread(self._log_detail, "Schema Post-Data", c0, o0)
            if not s0:
                log_msg(f"✘ Schema POST-DATA migration failed: {m0}")
                update_ui("Post Migration Failed", m0, "red")
                return
            log_msg("✔ Step 4b: Schema POST-DATA migration completed.")

            # Post-sync steps
            # 8/9: Sequences
            s2, m2, c2, o2 = self.post_sync.sync_sequences()
            self.call_from_thread(self._log_detail, "Sync Sequences", c2, o2)
            log_msg(f"{'✔' if s2 else '✘'} Step 8/9: Sync Sequences completed.")

            # 11: MatViews
            s1, m1, c1, o1 = self.post_sync.refresh_materialized_views()
            self.call_from_thread(self._log_detail, "Refresh MatViews", c1, o1)
            log_msg(f"{'✔' if s1 else '✘'} Step 11: Refresh MatViews completed.")

            # 10: Triggers
            s3, m3, c3, o3 = self.post_sync.enable_triggers()
            self.call_from_thread(self._log_detail, "Enable Triggers", c3, o3)
            log_msg(f"{'✔' if s3 else '✘'} Step 10: Enable Triggers completed.")

            # NEW: Reassign Ownership
            target_owner = self.config.get_dest_dict().get('user', 'postgres')
            s4, m4, c4, o4 = self.post_sync.reassign_ownership(target_owner)
            self.call_from_thread(self._log_detail, "Reassign Ownership", c4, o4)
            log_msg(f"{'✔' if s4 else '⚠'} Reassign Ownership: {m4}")
            
            # Validation
            s_aud, m_aud, c_aud, o_aud, r_aud = self.validator.audit_objects()
            self.call_from_thread(self._log_detail, "Object Audit", c_aud, o_aud)
            log_msg(f"  - Object Audit: {'OK' if s_aud else 'FAIL'}")
            ...
            s_rows, m_rows, c_rows, o_rows, r_rows = self.validator.compare_row_counts(use_stats=use_stats)
            self.call_from_thread(self._log_detail, "Row Parity", c_rows, o_rows)
            log_msg(f"  - Row Parity ({'stats' if use_stats else 'count'}): {'OK' if s_rows else 'FAIL'}")
            log_msg("✔ Steps 13/14: Validation completed.")
