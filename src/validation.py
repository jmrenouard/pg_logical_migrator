...
class Validator(SchemaFilterMixin):
    ...
    def compare_row_counts(self, use_stats=False):
        ...
        logging.info(
            f"[BOTH] Comparing row counts between source and destination ({mode_label})...")
        ...
        try:
            if use_stats:
                ...
                for schema, name in all_keys:
                    ...
                    outs.append(
                        f"{schema}.{name}: source~{s_count}  dest~{d_count}  diff~{diff}  [{status}]")
            else:
                ...
                for row in tables:
                    ...
                    try:
                        ...
                        logging.error(
                            f"[BOTH] Error counting table {schema}.{name}: {e}")
    ...
    def audit_objects(self):
        ...
        logging.info(
            "[BOTH] Auditing object counts between source and destination...")
        sf = self._get_schema_filter()
        sf_ns = self._get_schema_filter(nspname_col="n.nspname")
        query = f"""
        SELECT 'TABLE' AS type, count(*)::int as count FROM pg_class c
            JOIN pg_namespace n ON n.oid = c.relnamespace
            WHERE c.relkind = 'r' AND n.nspname NOT IN ('pg_catalog', 'information_schema') {sf}
        UNION ALL
        SELECT 'VIEW' AS type, count(*)::int as count FROM pg_class c
            JOIN pg_namespace n ON n.oid = c.relnamespace
            WHERE c.relkind = 'v' AND n.nspname NOT IN ('pg_catalog', 'information_schema') {sf}
        UNION ALL
        SELECT 'INDEX' AS type, count(*)::int as count FROM pg_class c
            JOIN pg_namespace n ON n.oid = c.relnamespace
            WHERE c.relkind = 'i' AND n.nspname NOT IN ('pg_catalog', 'information_schema') {sf}
        UNION ALL
        SELECT 'SEQUENCE' AS type, count(*)::int as count FROM pg_class c
            JOIN pg_namespace n ON n.oid = c.relnamespace
            WHERE c.relkind = 'S' AND n.nspname NOT IN ('pg_catalog', 'information_schema') {sf}
        UNION ALL
        SELECT 'FUNCTION' AS type, count(*)::int as count FROM pg_proc p
            JOIN pg_namespace n ON n.oid = p.pronamespace
            WHERE n.nspname NOT IN ('pg_catalog', 'information_schema') {sf_ns};
        """
        ...
        return True, "Object audit completed", [f"[BOTH] {query}"], [
            "Source counts: " + str(s_counts) + "\nDest counts: " + str(d_counts)], report
