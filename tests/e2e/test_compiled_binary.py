...
def test_binary_glibc_backward_compatibility():
    ...
    if not os.path.exists(dist_dir):
        pytest.skip("dist directory not found. Please compile the binary first.")
    
    binaries = [f for f in os.listdir(dist_dir) if f.startswith("pg_migrator")]
    if not binaries:
        pytest.skip("Compiled binary not found in dist/. Please run pyinstaller first.")
    ...
    # If the glibc is incompatible, it will fail with an error like "version `GLIBC_2.38' not found"
