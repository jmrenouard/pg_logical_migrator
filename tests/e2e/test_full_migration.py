...
def test_full_migration_e2e(tmp_path):
    ...
    python_bin = "venv/bin/python" if os.path.exists(
    ...
    lob_row = dc.execute_query(
        "SELECT picture_oid FROM schema_lobs.table_with_lobs WHERE id = 1;")[0]
    new_oid = lob_row['picture_oid']
    assert new_oid is not None
    assert new_oid != 0

    res_dest_lo = dc.execute_query(
    ...
    res_src_sub = sc.execute_query(
        f"SELECT count(*) FROM pg_subscription WHERE subname = '{sub_rev_name}';")
    ...
    cmd_cleanup_rev = [
        ...
        "-c",
        config_path]
    res_cleanup_rev = subprocess.run(
        cmd_cleanup_rev, capture_output=True, text=True)
    assert res_cleanup_rev.returncode == 0

    # Check if sub_rev is gone on SOURCE
    res_src_sub_after = sc.execute_query(
        f"SELECT count(*) FROM pg_subscription WHERE subname = '{sub_rev_name}';")
    assert res_src_sub_after[0]['count'] == 0

    print(
        f"E2E Success: {s_count} actors migrated and Rollback configured and cleaned up.")
