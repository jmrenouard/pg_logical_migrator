...
def test_cmd_init_replication_success(
        mock_rg, mock_val, mock_ps, mock_mig, mock_dbck, mock_bc, mock_cfg, mock_sl, mock_srd, mock_args):
    ...
    mock_dbck_instance.check_connectivity.return_value = {
    ...
    mock_val_instance.compare_row_counts.return_value = (True, "msg", [], [], [
                                                         {"table": "public.test", "source": 100, "dest": 100, "diff": 0, "status": "OK"}])
...
@patch("src.cli.pipelines.Config")
@patch("src.cli.pipelines.build_clients")
def test_cmd_init_replication_dry_run(
        mock_bc, mock_cfg, mock_sl, mock_srd, mock_args):
    mock_srd.return_value = "dummy_dir"
...
@patch("src.cli.pipelines.Validator")
@patch("src.cli.pipelines.ReportGenerator")
def test_cmd_post_migration_success(
        mock_rg, mock_val, mock_ps, mock_mig, mock_dbck, mock_bc, mock_cfg, mock_sl, mock_srd, mock_args):
    mock_srd.return_value = "dummy_dir"
...
@patch("src.cli.pipelines.build_clients")
@patch("src.cli.pipelines.DBChecker")
def test_cmd_init_replication_fail_connectivity(
        mock_dbck, mock_bc, mock_cfg, mock_sl, mock_srd, mock_args):
    mock_srd.return_value = "dummy_dir"
    mock_bc.return_value = (MagicMock(), MagicMock())
...
@patch("src.cli.pipelines.build_clients")
@patch("src.cli.pipelines.DBChecker")
def test_cmd_post_migration_fail_connectivity(
        mock_dbck, mock_bc, mock_cfg, mock_sl, mock_srd, mock_args):
    mock_srd.return_value = "dummy_dir"
    mock_bc.return_value = (MagicMock(), MagicMock())
...
@patch("src.cli.pipelines.Config")
@patch("src.cli.pipelines.build_clients")
def test_cmd_post_migration_dry_run(
        mock_bc, mock_cfg, mock_sl, mock_srd, mock_args):
    mock_srd.return_value = "dummy_dir"
    mock_bc.return_value = (MagicMock(), MagicMock())
    mock_args.dry_run = True
