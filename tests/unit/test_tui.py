...
from src.tui import MigratorApp, HistoryItem, main
...
@pytest.mark.asyncio
async def test_app_startup_and_history(mock_config, mock_pgclient, mock_dbchecker, mock_migrator, mock_postsync, mock_validator):
    app = MigratorApp("dummy.ini")
    
    mock_dbchecker_instance = mock_dbchecker.return_value
    mock_dbchecker_instance.check_connectivity.return_value = {"source": True, "dest": False}
...
@pytest.mark.asyncio
async def test_app_buttons(mock_config, mock_pgclient, mock_dbchecker, mock_migrator, mock_postsync, mock_validator):
    app = MigratorApp("dummy.ini")
    ...
    mock_val_instance.compare_row_counts.return_value = (True, "msg", [], [], [{'table': 't1', 'diff': 0, 'status': 'OK'}])
...
@pytest.mark.asyncio
async def test_app_progress_none(mock_config, mock_pgclient, mock_dbchecker, mock_migrator, mock_postsync, mock_validator):
    app = MigratorApp("dummy.ini")
    mock_mig_instance = mock_migrator.return_value
...
@pytest.mark.asyncio
async def test_app_pipelines(mock_config, mock_pgclient, mock_dbchecker, mock_migrator, mock_postsync, mock_validator):
    app = MigratorApp("dummy.ini")
...
@pytest.mark.asyncio
async def test_app_exception_handling(mock_config, mock_pgclient, mock_dbchecker, mock_migrator, mock_postsync, mock_validator):
    app = MigratorApp("dummy.ini")
    
    mock_dbchecker_instance = mock_dbchecker.return_value
    mock_dbchecker_instance.check_connectivity.side_effect = Exception("Test Exception")
...
@pytest.mark.asyncio
async def test_app_pipeline_exceptions(mock_config, mock_pgclient, mock_dbchecker, mock_migrator, mock_postsync, mock_validator):
    app = MigratorApp("dummy.ini")
    
    mock_mig_instance = mock_migrator.return_value
    # Force step 6 to fail
    mock_mig_instance.step6_setup_destination.side_effect = Exception("Sub fail")
    ...
    async with app.run_test() as pilot:
        ...
        mock_mig_instance.step4a_migrate_schema_pre_data.side_effect = Exception("Init fail")
        app.handle_buttons(MockButtonEvent("cmd_init"))
        await pilot.pause(0.1)
