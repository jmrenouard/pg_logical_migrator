...
def test_sync_large_objects_success():
    mock_config = MagicMock()
    mock_config.get_source_dict.return_value = {"host": "h1", "port": 5432, "user": "u1", "password": "p1", "database": "src"}
    mock_config.get_dest_dict.return_value = {"host": "h2", "port": 5433, "user": "u2", "password": "p2", "database": "dst"}
    mock_config.get_replication.return_value = {"publication_name": "pub", "subscription_name": "sub"}
    mock_config.get_target_schemas.return_value = ["all"]
    ...
    with patch("src.migrator.PostgresClient") as mock_pc, \
         patch("src.migrator.psycopg.connect") as mock_connect:
